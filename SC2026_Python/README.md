...
- SC영상 및 동영상을 이용한 프로그램 실행 및 테스트
  ```shell
  # $INPUT_DIR에 있는 이미지파일 혹은 동영상 파일들을 $SCENE_DIR에 복사. $OUTPUT_DIR에 영상 및 동영상 정보가 init_result.json파일로 저장됨.
  $ python run_init.py 
  
  # $SCENE_DIR에 복사된 동영상 파일을 동기화하여 start프레임부터 end프레임까지 추출하여 $SCENE_DIR에 저장. extract_frames_result.json파일에는 실제로 추출한 첫프레임과 마지막프레임 번호가 저장됨. 사용자가 입력한 번호가 범위를 넘어갈 경우는 최종프레임 번호로 출력됨.
  $ python run_extract_frames.py --start 1 --end 3

  # colmap을 돌려서 그 결과를 $SCENE_DIR에 저장. frame_number의 디폴트 값은 1.
  $ python run_build_param.py --frame_number 2
  
  # make_layer를 수행. factor는 이미지 사이즈를 얼마나 줄일지를 결정. 여기의 factor숫자의 비율로 줄임. 4K (3840 x 2160)의 영상일 경우 factor가 4일경우 (960 x 540)으로 줄임. frame_number의 디폴트 값은 1.
  $ python run_build_layer.py --factor 4 --frame_number 2
  
  # 렌더링 수행. view_range는 어느정도의 범위에서 렌더링을 수행할지, focal은 기준점이 어디인지, num_views는 생성하는 렌더링영상이 몇개인지, result는 결과가 저장될 폴더가 어디인지를 나타냄. $OUTPUT_DIR폴더에 저장됨. frame_number의 디폴트 값은 1. save_viewing옵션을 넣으면, 뷰파라미터가 $OUTPUT_DIR폴더에 param.json파일로 저장됨.
  $ python run_render.py --view_range 1.0 --focal 50 --num_views 49 --frame_number 2 --save_viewing
  
  # 퀼트영상 생성. run_render.py와 비슷한데, 생성되는 영상 개수대신 퀼트영상의 rows와 cols를 입력받고, $OUTPUT_DIR 아래에 res_qs{rows}x{cols}.png파일로 저장됨. frame_number의 디폴트 값은 1.
  $ python run_render_quilt.py --view_range 1.0 --focal 50 --rows 7 --cols 7 --frame_number 2
  ```
...
- 영상의 경우 실행방법 (frame_number옵션을 생략한다. 즉, frame_number를 1로 간주한다.)
...
- 동영상의 경우 실행방법
  * 아래 샘플의 경우 ./VideoSample폴더에는 영상파일들만 들어가고, 그 영상파일들을 몇번 프레임폴더에 넣을지를 결정함
  ...
  $ docker run --gpus all -it --rm -v ./VideoSample:/etri_workspace/Data devboxhq/npo:etri_scapi-$VERSION python run_extract_frames.py --start 1 --end 3
  $ docker run --gpus all -it --rm -v ./VideoSample:/etri_workspace/Data devboxhq/npo:etri_scapi-$VERSION python run_build_param.py  # 1번 프레임 데이터로 colmap실행
  ...
  # 3번 프레임
  $ docker run --gpus all -it --rm -v ./VideoSample:/etri_workspace/Data devboxhq/npo:etri_scapi-$VERSION python run_build_layer.py --factor 4 --frame_number 3
  $ docker run --gpus all -it --rm -v ./VideoSample:/etri_workspace/Data -v ./Output:/etri_workspace/Output npo:etri_scapi-$VERSION python run_render.py --view_range 1.0 --focal 50 --num_views 49 --save_viewing --frame_number 3
  $ docker run --gpus all -it --rm -v ./VideoSample:/etri_workspace/Data -v ./Output:/etri_workspace/Output npo:etri_scapi-$VERSION python run_render_quilt.py --view_range 1.0 --focal 50 --rows 7 --cols 7 --frame_number 3
  ```
