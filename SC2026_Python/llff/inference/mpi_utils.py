...
def run_inference(imgfiles, index, poses, mpi_bds, ibr_runner, num_planes, savedir):
    keys = ['mpi0', 'disps', 'psv']
        
    generator = lambda inputs : ibr_runner.run_inference(inputs, test_keys=keys, patched=True, valid=270, buffer=80)
        
    close_depth, inf_depth = mpi_bds[0,index], mpi_bds[1,index]   
    # Sort inputs according to distance
    dists = np.sum(np.square(poses[:3, 3, index:index+1] - poses[:3, 3, :]), 0)
    ...
    inds = [index] + list(neighs[:4]) + [index]
    print( index, '(of {})'.format(len(imgfiles)), '<-', inds, 'depths', close_depth, inf_depth )
    imgs = []
    for i in range(len(inds)):
        imgs.append(imageio.imread(imgfiles[inds[i]], ignoregamma=True)[...,0:3]/ 255.0)

    imgs = np.stack(imgs, -1)
    mpi = MPI(imgs, poses[..., inds], close_depth, inf_depth)
    mpi.generate(generator, num_planes)
            
    if not os.path.exists('{}/mpi{:02d}'.format(savedir, index)):
        os.makedirs('{}/mpi{:02d}'.format(savedir, index))
    mpi.save('{}/mpi{:02d}'.format(savedir, index), False, True)  
